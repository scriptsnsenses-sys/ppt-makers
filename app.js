...
/* =========================================================================
 * DGSL (DG Slide Language) v1 — declarative presentations as code.
 * Positions use a 1280x720 canvas. Compiled server-side into a native
 * editable .pptx. DGSL is data, not code: it is validated and compiled,
 * never executed, so no sandbox is needed.
 * ========================================================================= */

const MAX_DGSL_CHARS = 200000;

const DGSL_EXAMPLE = `presentation "AI Models" {
    ...
    slide "Title" {
        ...
        text "Comparing modern coding models" {
            x 240
            y 290
            width 800
            height 60
    ...
    slide "Scores" {
        ...
        chart bar {
            ...
            labels ["Muse", "Kimi", "DeepSeek", "GLM"]
            values [72.6, 71.8, 78.7, 69.9]
            color primary
        }
    }
    ...
    slide "Conclusion" {
        ...
        text "DeepSeek leads this dataset" {
            x 100
            y 90
            width 1000
            height 90
...
async function generateFromDGSL() {
  const editor = $("dgsl-editor");
  const dgsl = editor ? editor.value : "";
  if (!dgsl.trim()) {
    showStatus("error", "Write some DGSL first — or click Load example.");
    return;
  }
  if (dgsl.length > MAX_DGSL_CHARS) {
    showStatus("error", `DGSL is too long (${dgsl.length} chars, max ${MAX_DGSL_CHARS}).`);
    return;
  }

  setLoading(true);
  showStatus("info", "Compiling DGSL\u2026");

  try {
    const res = await fetch("/api/generate-dgsl", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ dgsl }),
    });
    ...
    const blob = await res.blob();
    downloadBlob(blob, filenameFromDisposition(res) || "presentation.pptx");
    const warnings = warningsFromHeader(res);
    showStatus("ok", warnings
      ? `PowerPoint generated \u2014 check your downloads. (${warnings})`
      : "PowerPoint generated \u2014 check your downloads.");
  } catch (e) {
    showStatus("error", "Network error: " + e.message);
  } finally {
    setLoading(false);
...
async function validateDGSL() {
  ...
  showStatus("info", "Validating DGSL\u2026");
  try {
    const res = await fetch("/api/dgsl-validate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ dgsl }),
    });
    ...
    if (!res.ok || !data.ok) {
      const details = Array.isArray(data.details) && data.details.length
        ? " \u2014 " + data.details.join("; ")
        : "";
      showStatus("error", (data.error || "Validation failed") + details);
      return;
    }
    const parts = [];
    if (data.stats) {
      parts.push(`${data.stats.slides || 0} slides`);
      parts.push(`${data.stats.elements || 0} elements`);
    }
    const warn = Array.isArray(data.warnings) && data.warnings.length
      ? ` (${data.warnings.length} warning${data.warnings.length === 1 ? "" : "s"}: ${data.warnings.slice(0, 3).join("; ")}${data.warnings.length > 3 ? "; \u2026" : ""})`
      : "";
    showStatus("ok", `DGSL is valid — ${parts.join(", ")}.${warn}`);
  } catch (e) {
    showStatus("error", "Network error: " + e.message);
  }
}
...
function warningsFromHeader(res) {
  try {
    const raw = res.headers.get("X-DGSL-Warnings") || "";
    if (!raw) return "";
    const list = JSON.parse(raw);
    if (!Array.isArray(list) || !list.length) return "";
    return `${list.length} warning${list.length === 1 ? "" : "s"}`;
  } catch (_) {
    return "";
  }
}
...
function updateDGSLCount() {
  const editor = $("dgsl-editor");
  const count = $("dgsl-count");
  if (editor && count) {
    count.textContent = `${editor.value.length.toLocaleString()} / ${MAX_DGSL_CHARS.toLocaleString()}`;
    count.style.color = editor.value.length > MAX_DGSL_CHARS ? "#dc2626" : "";
  }
}

let dgslSaveTimer = null;
function scheduleDGSLSave() {
  if (dgslSaveTimer) clearTimeout(dgslSaveTimer);
  dgslSaveTimer = setTimeout(() => {
    try {
      const editor = $("dgsl-editor");
      if (editor) localStorage.setItem("ppt-maker-dgsl-v1", editor.value);
    } catch (_) { /* non-fatal */ }
  }, 300);
}

function initDGSLEditor() {
  const editor = $("dgsl-editor");
  if (!editor) return;
  let saved = null;
  try {
    saved = localStorage.getItem("ppt-maker-dgsl-v1");
  } catch (_) { /* ignore */ }
  editor.value = saved !== null && saved !== undefined ? saved : DGSL_EXAMPLE;
  ...
  const exampleBtn = $("dgsl-example");
  if (exampleBtn) {
    exampleBtn.addEventListener("click", () => {
      editor.value = DGSL_EXAMPLE;
      updateDGSLCount();
      scheduleDGSLSave();
      showStatus("ok", "Example DGSL loaded — press Generate to export it.");
    });
  }
  const validateBtn = $("dgsl-validate");
  if (validateBtn) validateBtn.addEventListener("click", validateDGSL);
  const clearBtn = $("dgsl-clear");
  if (clearBtn) {
